    ...
    create or replace table `data-platform-455517`.`stripe`.`int__stripe_substack_charges`
    ...
    OPTIONS(
      description="""Subscriber-level Stripe charges with revenue recognition applied. Annual subscriptions are spread across 12 rows (one per reporting month). Reporting month follows the 16th-to-15th financial calendar. Includes all charges with a linked invoice, including charges for churned subscribers.\n"""
    )
    as (
