    ...
    create or replace table `data-platform-455517`.`stripe`.`fct__stripe_substack_month_revenue_projections`
    ...
    OPTIONS(
      description="""One-month forward revenue projection for the next financial reporting period. Combines three revenue streams: monthly Stripe subscriber renewals (scheduled renewal window), annual Stripe subscriber ongoing recognition (all active annual subs at 1/12 of annual fee), and App Store estimate (prior month payout carried forward). Churn and new acquisition adjustments are applied to subscriber rows using median counts from the last 3 completed reporting months. All rows carry projection_type = 'projected'.\n"""
    )
    as (
...
actuals_base as (
    ...
    from `data-platform-455517`.`stripe`.`fct__stripe_substack_month_accrual`
...
starts_revenue as (
    ...
    from `data-platform-455517`.`stripe`.`fct__stripe_substack_charge_accrual` ca
    join `data-platform-455517`.`stripe`.`int__stripe_substack_subscriptions` s
...
live_revenue as (
    ...
    from `data-platform-455517`.`stripe`.`fct__stripe_substack_charge_accrual`
    cross join reporting_window rw
...
select
    ...
    re.prior_revenue                                      as recurring_revenue,
